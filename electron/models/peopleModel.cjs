...
const PeopleModel = {
  ...
  async listPaged(filters = {}) {
    ...
    if (filters.team_id) {
      whereSql += ` AND team_id = $${paramIndex++}`;
      params.push(filters.team_id);
    }
    
    if (filters.search) {
      whereSql += ` AND (first_name ILIKE $${paramIndex} OR last_name ILIKE $${paramIndex} OR email ILIKE $${paramIndex})`;
      params.push(`%${filters.search}%`);
      paramIndex++;
    }

    const countResult = await db.query(
      `SELECT COUNT(*)::int AS count FROM people ${whereSql}`,
      params
    );
    const total = countResult.rows[0]?.count ?? 0;

    const result = await db.query(
      `SELECT * FROM people ${whereSql} ORDER BY last_name, first_name LIMIT $${paramIndex++} OFFSET $${paramIndex++}`,
