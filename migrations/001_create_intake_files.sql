...
CREATE TRIGGER IF NOT EXISTS trigger_intake_files_updated_at
  AFTER UPDATE ON intake_files
  FOR EACH ROW
  BEGIN
    UPDATE intake_files SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
  END;
